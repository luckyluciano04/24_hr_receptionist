...
export async function GET(request: NextRequest) {
  try {
    ...
    if (callsError) {
      logger.error('appointments.calls_fetch_error', { error: String(callsError) });
    ...
    if (error) {
      logger.error('appointments.fetch_error', { error: String(error) });
      return NextResponse.json({ error: 'Failed to fetch appointments' }, { status: 500 });
    }

    return NextResponse.json({ appointments: appointments ?? [] });
  } catch (error) {
    logger.error('appointments.get_error', { error: String(error) });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      logger.error('appointments.insert_error', { error: String(error) });
      return NextResponse.json({ error: 'Failed to create appointment' }, { status: 500 });
    }

    return NextResponse.json({ appointment: data }, { status: 201 });
  } catch (error) {
    logger.error('appointments.post_error', { error: String(error) });
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    logger.error('appointments.patch_error', { error: String(error) });
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
